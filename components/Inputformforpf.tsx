...
const Inputformforpf = ({
  ...
  heading: string;
  title: string;
  method: (plain_text: string, key: string) => string;
  ...
  const formsubmithandler = (form: FormEvent): void => {
    try {
      form.preventDefault();
      const val = method(inputvalue, keyvalue);
      returner(val);
  ...
  return (
    ...
    >
      ...
      <div className="w-full">
        ...
        <div
          className={`grid ${
            errormessage === "" ? "grid-rows-[0fr]" : "grid-rows-[1fr]"
